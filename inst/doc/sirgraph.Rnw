...
Further, this function returns 0.9 if the vertex is an unvaccinated vertex, and 0.2 if it is vaccinated. This
function ignores the time, and note it is vectorised over the vertices by using the {\tt ifelse} function:
...
Now we can use either of these to build a spreader function using {\tt spreadF}, along with a constant function
for the recovery probability. We'll use a tree network to see how vaccinated nodes isolate branches of the
tree. 
...
is returned in its final state with infection and removal times as node
attributes.
