...
One thing we haven't done here is to update the time attribute of the graph. This can be done
within a loop that repeatedly calls the spreader function, but first we need to look at stopper functions.
...
starter function, a spreader function, and a stopper function, and
...
The infection is clearly still going at that point. Now we'll start again and it run until 
...
has been influenced by the idea of {tt dependency injection}, so that the function code
